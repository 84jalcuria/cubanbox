    ...
    <div className='fixed z-50 inset-0 bg-gray-800/40 overflow-auto flex justify-center items-center'>
      <div
        className={`mt-10 p-4 bg-[#262C34] w-[90%] max-w-sm rounded-xl flex flex-col justify-center items-stretch space-y-3
          ${
            submitting || uploadingAvatar || downloadingAvatar
              ? 'animate-pulse'
              : null
          }
          `}
      >
        ...
        <ButtonClose
          onClose={close}
          disabled={submitting || uploadingAvatar || downloadingAvatar}
        />
        {/*-------------------------Html Form--------------------------*/}
        <form
        ...
        >
          ...
          <div className='text-white w-full text-center'>
            ...
            {/*----------------------Avatar uploading---------------------*/}
            {uploadingAvatar && (
              <h1 className='mt-4 text-sm sm:text-sm font-light'>
                Subiendo Avatar
              </h1>
            )}
            {/*----------------------Avatar downloading-------------------*/}
          ...
          <Avatar
            ...
            onUploading={(state) => setUploadingAvatar(state)}
            onDownloading={(state) => setDownloadingAvatar(state)}
          />
          {/*-----------------------Inputs-----------------------------*/}
          <InputEmail
